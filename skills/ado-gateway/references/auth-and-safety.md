...
- Write flows are limited to the dedicated scripts for creating work items, creating work item comments, creating pull requests, and adding or replying to pull request comments.
- Every write script must default to dry-run and require both `--execute` and `--confirm-write WRITE_TO_ADO` before network mutation.
